...
class CarrotMan:
  ...
  def make_tmux_data(self):
    try:
      subprocess.run(
          "rm /data/media/tmux.log; tmux capture-pane -pq -S-1000 > /data/media/tmux.log",
          shell=True,
          capture_output=True,
          text=False
      )
      subprocess.run(
        "/data/openpilot/selfdrive/apilot.py",
        shell=True,
        text=False
      )
    except Exception:
    ...
    while True:
      try:
        ...
        if json_obj is None:
          ...
          try:
            result = subprocess.run(json_obj['echo_cmd'], shell=True, text=False)
            try:
  ...
  def carrot_route(self):
    ...
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
      ...
      while True:
        print("################# waiting for CarrotMan route #####################")
        conn, addr = s.accept()
        with conn:
